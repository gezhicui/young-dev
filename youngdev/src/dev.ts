...
export const dev = async () => {
  ...
  app.use(`/youngdev`, express.static(path.resolve(__dirname, 'client')));

  const youngdevServe = createServer(app);
  const ws = createWebSocketServer(youngdevServe);
  ...
  const buildMain = async ({ appData }: { appData: AppData }) => {
    // 获取用户数据
    const userConfig = await getUserConfig({ appData, youngdevServe });
    // 获取用户配置的mock数据
    const mockConfig = await getMockConfig({ appData, youngdevServe });
    // 获取 routes 配置
  ...
  youngdevServe.on('REBUILD', async ({ appData }) => {
    await buildMain({ appData });
    sendMessage('reload');
  });

  youngdevServe.listen(port, async () => {
